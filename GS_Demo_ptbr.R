...
df.blups[1:10,]
mean(df.blups[1:10,]$blups) # Ganho gen?tico


################################################################################
######################  Predi??o Gen?mica 1 ambiente ###########################
################################################################################
...
################################################################################
######################  Sele??o Gen?mica multi-ambiente #######################
################################################################################
...
# y = u + a + g + gxa + e --> M?dias j? diluidas

mmes.G2 <- mmes(value ~ 1 ,   #y = a + [...]
                random = ~ env + vsm(ism(gid), Gu = G) + env:gid, # [...] + g + gxa + e
                data = data,getPEV = T)
...
# Selecionando os 10 melhores
df.blups[1:10,]
mean(df.blups[1:10,]$mu) # Ganho gen?tico
...
df<-read.csv('Env_data.csv',sep=';')

#Processamento por janela de crescimento
