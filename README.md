...
# fold(fold.n, n)
#
# - fold.n: number of partitions (folds) for cross-validation
# - n: total number of observations to be split into folds
#
# Returns a list with indices of observations assigned to each fold.

# mmes.fold function for single environments
# mmes.fold(dados, G, fold)
#
# - dados: data frame with phenotypic data containing columns 'gid' and 'value'
# - G: genomic additive relationship matrix with row and column names matching 'gid'
# - fold: list of folds generated by fold() function
#
# Performs cross-validation by fitting mixed models masking observations in each fold.
# Returns predictive accuracy (correlation between predicted and observed values) per fold.

# mmesCV function for multi-environments
# mmesCV(dados, G = NULL, W = NULL, CV = NULL, fold.n = 5, looEnv = TRUE, covGE = FALSE)
#
# - dados: phenotypic data frame containing columns 'gid' and 'env'
# - G: genomic additive relationship matrix
# - W: environmental similarity matrix
# - CV: cross-validation scheme identifier ('0', '1', '2', '00')
# - fold.n: number of folds for CV (default 5)
# - looEnv: for CV = '00' only; indicates if leave-one-out cross-validation is applied to environments,
#           while genotypes are partitioned into folds defined by fold.n
# - covGE: logical, whether to build G×E covariance structures following Jarquín et al. (2014)
#
# Supports multiple CV schemes combining genotypes and environments.
# Fits mixed models with genomic and environmental covariance.
# Returns accuracy metrics per fold.
