...
# Performs cross-validation for single-environment data.
# In each iteration, fits a mixed model using mmes(), masking phenotypes in the current fold.
# Returns predictive accuracy (Pearson correlation between predicted and observed values) per fold.
